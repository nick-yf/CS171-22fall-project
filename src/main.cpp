...
//    glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);
    ...
    //   glGetIntegerv(GL_VIEWPORT, data);
      Vec3 pos = glm::unProject(Input::mouse_position, scene.camera.transform.ModelMat() * scene.camera.LookAtMat(), scene.camera.PerspectiveMat(), Vec4{0, 0, window_width, window_height});
    ...
    //   Vec4 cursor_world = glm::inverse(scene.camera.LookAtMat()) * glm::inverse(scene.camera.PerspectiveMat()) * cursor_pos;
      ...
      if(Input::GetMouseButton(0)){
        cloth->fixPointByCursor(origin, dir, scene.camera.transform.Forward(), true);
      }
      else{
        cloth->fixPointByCursor(origin, dir, scene.camera.transform.Forward(), false);
      }
      /// fixed update
